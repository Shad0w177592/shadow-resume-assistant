...
export function WorkbenchPage() {
  ...
  const [pendingExportBlocked, setPendingExportBlocked] = useState(false);
  ...
  useEffect(() => {
    ...
    apiRequest<EditProposal[]>(`/api/jobs/${jobId}/edit-proposals/pending`).then((items) => setProposal(items[0] || null)).catch(() => undefined);
  }, [jobId, notify]);
  ...
  function editSectionTitle(sectionId: string, sectionKey: string, title: string) {
    if (!draft) return;
    undoStack.current.push(structuredClone(draft)); redoStack.current = [];
    setDraft({ ...draft, document: { ...draft.document, sections: draft.document.sections.map((section) => section.section_id === sectionId ? { ...section, title } : section) } });
    updateSection(sectionKey, { title });
  }
  function editBlockHeading(blockId: string, heading: string) {
    if (!draft) return;
    undoStack.current.push(structuredClone(draft)); redoStack.current = [];
    setDraft({ ...draft, document: { ...draft.document, sections: draft.document.sections.map((section) => ({ ...section, blocks: section.blocks.map((block) => block.block_id === blockId ? { ...block, heading } : block) })) } });
  }

  function editGreetingMessage(value: string) {
    if (!draft) return;
    undoStack.current.push(structuredClone(draft)); redoStack.current = [];
    setDraft({ ...draft, document: { ...draft.document, greeting_message: value.slice(0, 142) } });
  }
  async function copyGreetingMessage() {
    const value = draft?.document.greeting_message?.trim();
    if (!value) { notify("请先生成或填写打招呼语", "error"); return; }
    try {
      await navigator.clipboard.writeText(value);
      notify("打招呼语已复制，可直接粘贴到 BOSS 直聘", "success");
    } catch {
      notify("复制失败，请选中文本后手动复制", "error");
    }
  }
  ...
  async function saveDraft() {
    if (!draft) return;
    if (config) await apiRequest(`/api/jobs/${jobId}/resume-config`, "PUT", { config });
    const saved = await apiRequest<ResumeDraft>(`/api/jobs/${jobId}/draft`, "PUT", { document: draft.document });
    setDraft(saved); notify("草稿已保存", "success");
  }
  ...
  async function exportBoth(saveFirst = false) {
    ...
    try {
      ...
      if (message.includes("未接受或拒绝的 AI 修改建议")) {
        setPendingExportBlocked(true);
        setShowVersions(false);
        setRightPanelCollapsed(false);
        const pending = await apiRequest<EditProposal[]>(`/api/jobs/${jobId}/edit-proposals/pending`).catch(() => []);
        setProposal(pending[0] || null);
        notify(pending.length ? "请先在右侧接受或拒绝这条 AI 修改，再重新导出" : message, "error");
  ...
  async function proposeEdit() {
    if (!selectedParagraph) { notify("请先在画布中选择标题或段落", "error"); return; }
  ...
  async function decide(action: "accept" | "reject") {
    ...
    if (action === "accept") {
      const savedDraft = await apiRequest<ResumeDraft>(`/api/jobs/${jobId}/draft`);
      setDraft(savedDraft);
      if (proposal.target_block_id.startsWith("section:")) {
        const sectionId = proposal.target_block_id.slice("section:".length);
        const changedSection = savedDraft.document.sections.find((section) => section.section_id === sectionId);
        if (changedSection) updateSection(changedSection.section_key, { title: changedSection.title });
      }
    }
    const pending = await apiRequest<EditProposal[]>(`/api/jobs/${jobId}/edit-proposals/pending`);
    setProposal(pending[0] || updated);
  }
  ...
  const ordered = [...config.sections].sort((a, b) => a.order - b.order);
  return (
    <main className={`workbench${leftPanelCollapsed ? " left-panel-collapsed" : ""}${rightPanelCollapsed ? " right-panel-collapsed" : ""}`} aria-label="简历工作台">
      <header className="workbench-toolbar">
        <div className="workbench-toolbar-heading"><span className="workbench-mark" aria-hidden="true">影</span><div><small>影子简历助手 · RESUME EDITOR</small><strong>简历工作台</strong></div></div>
        <div className="action-cluster primary-actions"><Button disabled={busy} onClick={() => setShowGenerateOptions(true)}>生成简历</Button><Button className="secondary" disabled={!draft || busy} onClick={() => setShowPolish(true)}>润色</Button></div>
        <div className="workbench-toolbar-config"><label>模板 <select value={config.template} onChange={(event) => patchConfig({ template: event.target.value as ResumeConfig["template"] })}><option value="single_column">简洁单栏</option><option value="technical_double_column">技术型双栏</option></select></label><label>页数 <select value={config.page_target} onChange={(event) => patchConfig({ page_target: Number(event.target.value) as 1 | 2 })}><option value={1}>一页（至少 80%）</option><option value={2}>两页（第二页过半）</option></select></label></div>
        <div className="workbench-toolbar-actions">
          <div className="action-cluster history-actions"><Button className="ghost" onClick={undo}>撤销</Button><Button className="ghost" onClick={redo}>重做</Button></div>
          <div className="action-cluster document-actions"><Button className="ghost" onClick={saveDraft} disabled={!draft}>保存草稿</Button><Button className="secondary" onClick={saveVersion} disabled={!draft}>保存版本</Button><Button className="ghost" onClick={() => setShowVersions((value) => !value)}>历史版本</Button><Button className="ghost" onClick={() => setParagraphsExpanded((value) => !value)} disabled={!draft}>{paragraphsExpanded ? "收起段落" : "展开段落"}</Button><Button className="ghost" onClick={() => exportBoth(false)} disabled={!draft}>导出</Button><Button onClick={() => exportBoth(true)} disabled={!draft}>保存并导出</Button></div>
        </div>
      </header>
      <aside className="workbench-left"><div className="panel-heading"><h2>栏目与取舍</h2><button type="button" className="panel-toggle" aria-label={leftPanelCollapsed ? "展开栏目与取舍" : "收起栏目与取舍"} onClick={() => setLeftPanelCollapsed((value) => !value)}>{leftPanelCollapsed ? "展开" : "收起"}</button></div><div className="panel-content"><p>拖动栏目可调整顺序；选择“不限”可保留全部条目。</p>{ordered.map((section) => <article className="section-option" key={section.section_key} draggable onDragStart={() => { dragKey.current = section.section_key; }} onDragOver={(event) => event.preventDefault()} onDrop={() => dropSection(section.section_key)}><label className="check"><input type="checkbox" checked={section.enabled} onChange={(event) => updateSection(section.section_key, { enabled: event.target.checked })} />{section.title}</label><label className="field section-limit"><span>使用多少条</span><select aria-label={`${section.title}使用条数`} value={section.max_entries ?? ""} onChange={(event) => updateSection(section.section_key, { max_entries: event.target.value ? Number(event.target.value) : null })}><option value="">不限</option>{Array.from({ length: 20 }, (_, index) => index + 1).map((count) => <option key={count} value={count}>{count} 条</option>)}</select></label>{config.template === "technical_double_column" && <select aria-label={`${section.title}栏位`} value={section.column} onChange={(event) => updateSection(section.section_key, { column: event.target.value as SectionConfig["column"] })}><option value="left">左栏</option><option value="right">右栏</option></select>}</article>)}<h3>经历取舍</h3>{profile.entries.map((entry) => <label className="entry-mode" key={entry.id}><span>{entry.title || "未命名经历"} · {entry.importance || 3}/5</span><select value={config.entry_modes[entry.id] || "ai_decide"} onChange={(event) => patchConfig({ entry_modes: { ...config.entry_modes, [entry.id]: event.target.value as ResumeConfig["entry_modes"][string] } })}><option value="ai_decide">AI 决定</option><option value="must_include">必须使用</option><option value="exclude_this_resume">不要用于这一份简历</option></select></label>)}</div></aside>
      <section className="workbench-canvas">{!draft ? <Card><EmptyState title="尚未生成草稿" description="完成左侧配置后生成简历。" /></Card> : <article className={`resume-preview ${config.template}`}><header><h1>{draft.document.personal_info.name || "姓名"}</h1><p>{draft.document.personal_info.contacts.join(" · ")}</p></header>{draft.document.sections.map((section) => <section className={`resume-section ${section.column || "full"}`} key={section.section_id}><h2><input aria-label={`编辑栏目标题${section.title}`} className={`resume-section-title-editor${selectedParagraph === `section:${section.section_id}` ? " selected" : ""}`} value={section.title} onFocus={() => setSelectedParagraph(`section:${section.section_id}`)} onChange={(event) => editSectionTitle(section.section_id, section.section_key, event.target.value)} /></h2>{section.blocks.map((block) => <div key={block.block_id}>{block.heading && <input aria-label={`编辑标题${block.heading}`} className={`resume-heading-editor${selectedParagraph === `heading:${block.block_id}` ? " selected" : ""}`} value={block.heading} onFocus={() => setSelectedParagraph(`heading:${block.block_id}`)} onChange={(event) => editBlockHeading(block.block_id, event.target.value)} />}{shouldShowBlockMeta(block.meta, block.paragraphs.map((paragraph) => paragraph.text)) && <small>{block.meta}</small>}{block.paragraphs.map((paragraph) => <ResumeParagraphEditor key={paragraph.paragraph_id} label={`编辑${block.heading || section.title}`} text={paragraph.text} expanded={paragraphsExpanded} selected={selectedParagraph === paragraph.paragraph_id} onFocus={() => setSelectedParagraph(paragraph.paragraph_id)} onChange={(value) => editParagraph(paragraph.paragraph_id, value)} />)}</div>)}</section>)}</article>}</section>
      <aside className="workbench-right"><div className="panel-heading"><h2>{showVersions ? "历史版本" : "AI 修改助手"}</h2><button type="button" className="panel-toggle" aria-label={rightPanelCollapsed ? "展开 AI 修改助手" : "收起 AI 修改助手"} onClick={() => setRightPanelCollapsed((value) => !value)}>{rightPanelCollapsed ? "展开" : "收起"}</button></div><div className="panel-content">{showVersions ? <div className="version-list">{versions.length === 0 ? <p>还没有主动保存的版本。</p> : versions.map((version) => <article key={version.id}><strong>{version.name}</strong><small>{new Date(version.created_at).toLocaleString()}</small>{version.notes && <p>{version.notes}</p>}<div className="actions"><Button className="ghost" onClick={() => editVersionMeta(version)}>改名/备注</Button><Button className="ghost" onClick={() => compareVersion(version)}>对比</Button><Button className="ghost" onClick={() => setRestoreTarget(version)}>恢复</Button><Button className="ghost" onClick={() => exportVersion(version)}>导出</Button><Button className="danger" onClick={() => deleteVersion(version)}>删除</Button></div></article>)}{comparison && <p>对比结果：{comparison.length ? `${comparison.length} 个内容块发生变化` : "没有变化"}</p>}</div> : <>{selectedParagraph ? <p className="selection-ready">{selectedParagraph === "greeting" ? "已选择打招呼语" : (selectedParagraph.startsWith("section:") ? "已选择一个栏目标题" : (selectedParagraph.startsWith("heading:") ? "已选择一个条目标题" : "已选择一个段落"))}</p> : <p>先在画布中选择标题、段落或打招呼语。</p>}<label className="field"><span>修改要求</span><textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="例如：写得更简洁，降低夸张程度" /></label><div className="actions"><Button className="secondary" onClick={recording ? stopRecording : startRecording}>{recording ? "停止录音" : "语音输入"}</Button><Button onClick={proposeEdit}>生成修改</Button></div><label className="field"><span>接受后保存到</span><select value={saveScope} onChange={(event) => setSaveScope(event.target.value)}><option value="current_resume">只用于当前简历</option><option value="also_profile">同时保存到个人资料库</option></select></label>{proposal && <Card title="修改前后对比"><div className="proposal-before"><small>修改前</small><p>{proposal.before_text}</p></div><div className="proposal-after"><small>修改后</small><p>{proposal.after_text}</p></div><p><strong>理由：</strong>{proposal.payload.reason}</p><p><strong>证据：</strong>{proposal.payload.evidence_ids.join("、")}</p>{proposal.status === "pending" ? <div className="actions"><Button onClick={() => decide("accept")}>接受</Button><Button className="danger" onClick={() => decide("reject")}>拒绝</Button><Button className="ghost" onClick={proposeEdit}>重新生成</Button></div> : <p className="save-state">已{proposal.status === "accepted" ? "接受" : "拒绝"}</p>}</Card>}<section className="greeting-message-card" aria-labelledby="greeting-message-title"><div className="greeting-message-heading"><h3 id="greeting-message-title">打招呼语</h3><span>{draft?.document.greeting_message?.length || 0}/142</span></div><p>用于 BOSS 直聘首次沟通，不会写入 Word 或 PDF 简历。可组合身份、相关经历、具体技能、岗位动机、软能力和到岗信息，建议写到 120–142 个字符；中文、英文、数字、空格和标点都计入，最多 142 个字符。</p><textarea aria-label="打招呼语" className={selectedParagraph === "greeting" ? "selected" : ""} maxLength={142} value={draft?.document.greeting_message || ""} onFocus={() => setSelectedParagraph("greeting")} onChange={(event) => editGreetingMessage(event.target.value)} placeholder="生成简历后，AI 会根据当前岗位生成打招呼语。" disabled={!draft} /><div className="actions"><Button className="secondary" onClick={copyGreetingMessage} disabled={!draft?.document.greeting_message?.trim()}>复制打招呼语</Button></div></section></>}</div></aside>
