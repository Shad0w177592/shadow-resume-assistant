...
test("backup restore and exact clear confirmation are available from settings", async () => {
  const request = vi.fn(async (path: string, method = "GET") => {
    if (path === "/api/settings") return { model: "gpt-5-mini", api_mode: "chat_completions", base_url: "https://gateway.example.com/v1", transcription_base_url: "https://speech.example.com/v1", transcription_model: "whisper-1" };
    if (path === "/api/bootstrap") return { data_directory: "C:\\data", api_key_configured: true, transcription_api_key_configured: true };
  ...
  expect(screen.getByLabelText("语音 Base URL（选填）")).toHaveValue("https://speech.example.com/v1");
