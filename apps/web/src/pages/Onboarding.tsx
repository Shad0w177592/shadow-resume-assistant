  ...
  async function next() {
    ...
    if (step === 1) {
      try {
        ...
        if (apiKey)
          await apiRequest("/api/credentials/openai", "PUT", {
            api_key: apiKey,
          });
      } catch (reason) {
      ...
      <Card>
        {step === 0 && (
          <>
            <h2>数据保存在这台电脑</h2>
            <p>
              个人资料、岗位、草稿和版本保存在本地；只有需要 AI
              分析或修改时，脱敏后的必要内容才会发送给你配置的模型服务。
            </p>
        ...
        {step === 1 && (
          <>
            <h2>配置 OpenAI、DeepSeek 或兼容网关</h2>
            <p>
              API Key 保存到 Windows
              凭据管理器，不写入数据库、日志或备份。现在也可以留空，稍后在设置中配置。
            </p>
            ...
            <small>
              {provider === "deepseek"
                ? "已使用 DeepSeek 官方地址；推荐 deepseek-v4-flash，也可选择 deepseek-v4-pro。"
                : "使用 JUAPI 时请选择 Chat Completions，并填写 https://www.juapi.net/v1。"}
            </small>
            <TextInput
              label="API Key（选填）"
              type="password"
        ...
        {step === 2 && (
          <>
            ...
            <div className="choice-grid">
              <button type="button">从空白开始</button>
              <button type="button">稍后导入简历</button>
            </div>
          </>
        )}
        {error && (
