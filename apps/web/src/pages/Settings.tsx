...
export function SettingsPage() {
  ...
  async function save() {
    try {
      ...
      if (transcriptionApiKey) {
        await apiRequest("/api/credentials/transcription", "PUT", {
  ...
  async function createBackup() {
    ...
    try {
      ...
      notify("备份已保存到本地", "success");
    } catch (error) {
      notify(error instanceof Error ? error.message : "备份失败", "error");
    } finally {
      setDataOperation(false);
    }
  }
  ...
  async function restoreBackup() {
    ...
    try {
      ...
      }>("/api/backups/restore", "POST", { path });
      setBackupPath(result.automatic_backup);
      notify(
        `已恢复 ${result.restored_files} 个文件；恢复前数据已自动备份`,
        "success",
      );
    } catch (error) {
      notify(
        error instanceof Error ? error.message : "恢复失败，原数据未改变",
        "error",
      );
    } finally {
      setDataOperation(false);
  ...
  async function clearAll() {
    try {
      await apiRequest("/api/data/clear", "POST", {
        confirmation: clearText,
        include_api_key: clearKey,
      });
      setClearText("");
      setConfigured(clearKey ? false : configured);
      setTranscriptionConfigured(clearKey ? false : transcriptionConfigured);
      notify("本地数据已全部清除", "success");
    } catch (error) {
    ...
    <main className="page" aria-labelledby="page-title">
      ...
      <div className="settings-grid">
        <Card title="AI 服务">
          ...
          <label className="field">
            ...
            <select
              ...
              value={provider}
              onChange={(event) => {
                const next = event.target.value as "openai" | "deepseek";
                setProvider(next);
                if (next === "deepseek") {
          ...
          <label className="field">
            ...
            >
              <option value="responses">Responses API（OpenAI 官方）</option>
              <option value="chat_completions">
                Chat Completions（JUAPI 等兼容网关）
              </option>
            </select>
          </label>
          <TextInput
          ...
          <small>
            {provider === "deepseek"
              ? "DeepSeek 官方使用 Chat Completions；推荐模型 deepseek-v4-flash，也可选择 deepseek-v4-pro。"
              : "OpenAI 官方请选择 Responses API；JUAPI 请选择 Chat Completions。请填写 API 根地址，不要附加具体接口路径。"}
          </small>
          <TextInput
            label="语音转写模型"
          ...
          />
          <small>
            OpenAI 官方推荐 gpt-transcribe；JUAPI
            等中转网关请填写其实际支持的语音模型，例如
            whisper-1。语音转写与上面的文本模型相互独立。
          </small>
          <TextInput
            label="替换 API Key（选填）"
            type="password"
        ...
        <Card title="语音转写服务">
          ...
          <TextInput
            label="语音 Base URL（选填）"
            placeholder="https://api.openai.com/v1"
            value={transcriptionBaseUrl}
            onChange={(event) => setTranscriptionBaseUrl(event.target.value)}
          />
          <small>
            留空会复用文本 Base URL。JUAPI 当前返回语音接口
            503；要使用语音，请填写真正支持 /audio/transcriptions
            的独立服务地址。OpenAI 官方地址为 https://api.openai.com/v1。
          </small>
          <TextInput
            label="语音转写模型"
            value={transcriptionModel}
          ...
          />
          <small>
            OpenAI 官方推荐 gpt-transcribe；其他服务请填写其实际支持的转写模型。
          </small>
          <TextInput
            label="语音 API Key（选填）"
            type="password"
          ...
          />
          <small>
            留空会复用上面的文本 API Key。独立语音 Key 只保存在 Windows
            凭据管理器，不写入数据库或备份。
          </small>
          <Button onClick={save}>保存语音设置</Button>
        </Card>
        <Card title="本地数据">
          <p className="path-text">{dataDirectory}</p>
          <p>
            可以改到 D
            盘等任意可写位置。请选择一个空文件夹；切换前会自动备份并复制数据，旧目录继续保留为安全副本。
          </p>
          <div className="actions">
            <Button disabled={dataOperation} onClick={changeDataDirectory}>
              更改保存位置
            </Button>
            <Button disabled={dataOperation} onClick={createBackup}>
              导出全部备份
            </Button>
            <Button
